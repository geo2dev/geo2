...
| Planned date | Date/time - format yyyy-MM-dd HH:mm, the 24-hour time portion HH:mm is optional | Date/time to indicate when the order is planned for inclusion in a route. It is affected by route optimization and calibration once an order is assigned to a route. Learn more about [Hub: Routes](../Hub_%20Routes/index.md). | N |
